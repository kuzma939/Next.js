...
export default function StatusLabel({children, status, disabled}: StatusLabelProps) {
    return(
        <span className={clsx(`inline-flex items-center py-1 px-3.5 rounded-3xl text-sm font-medium ${
        ...
        (status === Status.Pending && 'text-orange-700 bg-orange-100') ||
            ...
            ''}`)}>
                <div className="w-1 h-1 mr-2 rounded-full bg-current"/>{children}</span>
    )
}